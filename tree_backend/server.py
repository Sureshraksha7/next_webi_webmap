...
# BATCH NODE STATS
# -------------------------
@app.route("/batch_node_stats", methods=["POST"])
def batch_node_stats():
    data = request.get_json()
    if not data or 'nodeIds' not in data:
        return jsonify({"error": "Missing nodeIds in request"}), 400

    node_ids = data['nodeIds']
    if not isinstance(node_ids, list):
        return jsonify({"error": "nodeIds must be an array"}), 400

    if not node_ids:
        return jsonify({"stats": {}})

    # Convert to tuple for SQL IN clause and create placeholders
    ...
    try:
        with conn.cursor() as cur:
            ...
            inbound_query = f"""
                SELECT 
                    ...
                    COALESCE(SUM(count), 0) AS inbound_count
                FROM 
                    clicks
                WHERE 
                ...
                GROUP BY 
                    target_id
            """
            cur.execute(inbound_query, node_ids_tuple)
            for row in cur.fetchall():
                node_id = row['node_id']
                if node_id not in stats:
                    stats[node_id] = {'inboundCount': 0, 'outboundCount': 0}
                stats[node_id]['inboundCount'] = int(row['inbound_count'])

            # Get outbound counts
            outbound_query = f"""
                SELECT 
                    source_id AS node_id, 
                    COALESCE(SUM(count), 0) AS outbound_count
                FROM 
                    clicks
                WHERE 
                ...
                GROUP BY 
                    source_id
            """
            cur.execute(outbound_query, node_ids_tuple)
            for row in cur.fetchall():
                node_id = row['node_id']
                if node_id not in stats:
                    stats[node_id] = {'inboundCount': 0, 'outboundCount': 0}
                stats[node_id]['outboundCount'] = int(row['outbound_count'])

            # Ensure all requested nodes are in the response, even with zero counts
        ...
        print(f"Error in batch_node_stats: {str(e)}")
